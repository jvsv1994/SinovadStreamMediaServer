...
namespace SinovadMediaServer.Persistence.Repositories
{
    public class MediaItemRepository : GenericRepository<MediaItem>, IMediaItemRepository
    {
        ...
        {
            var list = await (from mediaItem in _context.MediaItems
            ...
            var listMediaItems = list.AsEnumerable().GroupBy(a => a.GenreId).SelectMany(x => x.DistinctBy(x => x.MediaItemId)).ToList();
            return listMediaItems;
        }


        public async Task<List<ItemDto>> GetItemsByLibrary(int libraryId)
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                              join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
        ...
        public async Task<List<ItemDto>> GetAllItemsByMediaType(MediaType mediaTypeId)
        {
            var list = await (from mediaItem in _context.MediaItems
                        join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                        join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
        ...
        public async Task<List<ItemDto>> GetAllItemsBySearchQuery(string searchQuery)
        {
            var list = await (from mediaItem in _context.MediaItems
                                        join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                                        join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
            ...
            var listMediaItems=list.AsEnumerable().GroupBy(a => a.MediaItemId).Select(x => x.First()).ToList();
            return listMediaItems;
        }

        // all items by media type
        public async Task<List<ItemDto>> GetAllItemsRecentlyAddedByMediaType(MediaType mediaTypeId)
        {
            var list = await (from mediaItem in _context.MediaItems
                                  join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                                  join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
            ...
            var listMediaItems = list.AsEnumerable().GroupBy(a => a.MediaItemId).Take(10).Select(x => x.First()).ToList();
            return listMediaItems;
        }

        public async Task<List<ItemDto>> GetAllItemsByMediaTypeAndProfile(MediaType mediaTypeId,int profileId)
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
        ...
        public async Task<List<ItemDto>> GetAllItemsRecentlyAdded()
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                              join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
        ...
        public async Task<List<ItemDto>> GetAllItemsByProfile(int profileId)
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                              join mediaFileProfile in _context.MediaFileProfiles on mediaFile.Id equals mediaFileProfile.MediaFileId
        ...
        public async Task<List<ItemDto>> GetItemsRecentlyAddedByLibrary(int libraryId)
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                              join mediaGenreItem in _context.MediaItemGenres on mediaItem.Id equals mediaGenreItem.MediaItemId
        ...
        public async Task<List<ItemDto>> GetItemsByLibraryAndProfile(int libraryId,int profileId)
        {
            var list = await (from mediaItem in _context.MediaItems
                              join mediaFile in _context.MediaFiles on mediaItem.Id equals mediaFile.MediaItemId
                              join mediaFileProfile in _context.MediaFileProfiles on mediaFile.Id equals mediaFileProfile.MediaFileId
